#!/usr/bin/env bash
# Builds the claude.ai artifact copy of the Feat Cards tool from the site page.
#
#   ./src/feat-cards/build-artifact.sh → src/feat-cards/artifact.html
#
# feat-cards.html is its own source (there are no parts to assemble). The artifact copy differs
...
title="$(grep -o '<title>[^<]*</title>' "$page" | head -1)"
{
  ...
  done | sed -e '/<nav class="toolbag"/,/<\/nav>/c\    <h1>Feat Cards <span class="sub">\&middot; PF2e</span></h1>'
} > "$out"
printf '%s  %s bytes\n' "$out" "$(wc -c < "$out")"
