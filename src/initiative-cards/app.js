(function(){
  ...
  var G = {
    ...
    /* speeds: the frame grows with the count. One speed sits alone, centred, in a narrow frame. With more,
       ...
       two or three speeds use the medium frame (the extras take the whole left side, or stack), four or
    ...
    spd:{y:50, h:40, tiers:[
      ...
      {n:2, x:132, w:50, path:"M0,0 L36,0 L50,20 L36,40 L0,40 Z", point:{x:21, y:0, w:25, h:40, size:13}, grid:{
  ...
  function cardHtml(d, edit, opts){
    ...
    return '<article class="card">' +
      ...
      '<div class="half stats">' +
        ...
        fld("hp", "big fit", "", " " + pos(G.heart.x, G.heart.y + 6, G.heart.w, 26)) +
  ...
  function speedFrame(n){
    ...
    return {x:t.x, y:G.spd.y, w:t.w, h:G.spd.h, path:t.path, point:t.point, grid:t.grid, hdrX:Math.min(t.x + t.w / 2 - G.hdr.w / 2, G.W - 5 - G.hdr.w)};   /* centred, but never past the card edge */
  }
  ...
  function drawCard(pdf, d, im, X, Y, blank){
    ...
    /* a filled accent bar with white Teko text, like the CSS .hdr / .lbl / .cap */
    var bar = function(label, x, y, w, h, size){
      accent(); rect(x, y, w, h, "F");
      pdf.setTextColor(255); font("bold", size, "Sub"); text(label, x + w / 2, y + h * 0.5 + size * 0.35, {align:"center"}); ink();
    };
    ...
    /* name and rule */
    accent(); centred(d.name, G.name.x, G.name.w, oy + G.name.y + 14, G.name.size, "Serif"); ink();
    ...
    centred(d.hp, G.heart.x, G.heart.w, oy + G.heart.y + 6 + 19, G.big.size);
    ...
    [["Fort","fort"],["Ref","ref"],["Will","will"]].forEach(function(q, k){
      ...
      bar(q[0], sv.x, y, sv.labelW, sv.rowH, sv.size);
    ...
    bar("Recall info", r2.x, ry, r2.w, r2.capH, 12);
